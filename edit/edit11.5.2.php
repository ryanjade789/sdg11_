...
if(isset($_GET['green_id'])) //the green_id came from 11.3.4.php which was declared as variable to edit record
    {
     ...
     $query = "SELECT * FROM `tbl11_5_2` WHERE `id` = $green_id";
 ...
 if (isset ($_POST['update'])){
  ...
  $points = ($ppa / 5) * 5;
      ...
      // SQL query to update the data in user table where the id = $green_id 
        $query ="UPDATE `tbl11_5_2` SET   `ppa` = '{$ppa}', `title` = '{$title}', `des` = '{$des}',`cost` = '{$cost}',`fund` = '{$fund}' WHERE `id`= $green_id";
...
<form method="POST">
  <h3 class="h3text" style="color: red; font-weight: bolder">Update Information</h3>
    <div class="form-group"><i class="fa fa-bar-chart"></i>
      ...
      <textarea class="form-control" id="ppa" name="ppa" rows="3" value="" required><?php echo $ppa ?></textarea>
    </div>
    ...
    <div class="form-group"><i class="fa fa-area-chart"></i>
      ...
      <textarea class="form-control" id="des" name="des" rows="5" value="" required><?php echo $des ?></textarea>
    </div>
    <div class="form-group"><i class="fa fa-area-chart"></i>
      <label for="cost">Total cost:</label>
      <textarea class="form-control" id="cost" name="cost" rows="5" value="" required><?php echo $cost ?></textarea>
    </div>
    <div class="form-group"><i class="fa fa-area-chart"></i>
      <label for="fund">Fund source:</label>
      <textarea class="form-control" id="fund" name="fund" rows="5" value="" required><?php echo $fund ?></textarea>
    </div>
...
<button type="reset" class="btn btn-danger mt-6 mb-3" name="cancel" onclick="window.location.href='../11.5.2.php';">
