...
if(isset($_GET['green_id'])) //the green_id came from 11.3.4.php which was declared as variable to edit record
    {
     ...
     $query = "SELECT * FROM `tbl11_5_3` WHERE `id` = $green_id";
 ...
 if (isset ($_POST['update'])){
  ...
  $points = ($ppa / 7) * 5;
      ...
      // SQL query to update the data in user table where the id = $green_id 
        $query ="UPDATE `tbl11_5_3` SET   `ppa` = '{$ppa}', `title` = '{$title}', `des` = '{$des}',`cost` = '{$cost}',`fund` = '{$fund}' WHERE `id`= $green_id";
...
<button type="reset" class="btn btn-danger mt-6 mb-3" name="cancel" onclick="window.location.href='../11.5.3.php';">
