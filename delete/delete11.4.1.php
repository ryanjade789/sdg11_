...
if (isset($_GET['delete'])) {
    ...
    if (isset($_GET['confirm']) && $_GET['confirm'] == 'true') {
        ...
        $query = "DELETE FROM `tbl11_4_1` WHERE `id` = {$deleteid}";
        $delete_query = mysqli_query($conn, $query);
        echo "<script type='text/javascript'>alert(' data deleted successfully!')</script>";
        header("Location: ../11.4.1.php");
    } else {
        // If not confirmed, show a confirmation dialog using JavaScript
        echo "<script>
              var confirmed = confirm('Are you sure you want to delete this record?');
              if (confirmed) {
                  window.location.href = 'delete11.4.1.php?delete=$deleteid&confirm=true';
              } else {
                  window.location.href = '../11.4.1.php';
              }
              </script>";
    }
}
